...
const ImageDrop: React.FC<Props> = ({ defaultImages, onDrop }) => {
    ...
    const grids = useMemo<(string | ReactNode)[][]>(() => {
        if (images) {
            ...
            return arrayChunks<string | ReactNode>(items, 7)
        }
        return []
    }, [images])
    ...
    const handleDrop = useCallback(
        (event: React.ChangeEvent<HTMLInputElement> | FileList | null) => {
            let newFiles: ImageFile[] = images
            if (event) {
                newFiles = [
                    ...newFiles.slice(0, newFiles.length - 1),
                    ...Array.from(
                        event instanceof FileList
                            ? event
                            : event.target.files || []
                    ).map((f) => ({ file: f })),
                    ...newFiles.slice(newFiles.length - 1),
                ]
                const placeholder =
                ...
                newFiles.length % 7 !== 0 &&
                !newFiles[newFiles.length - 1].placeholder
                    ? setImages([
                          ...newFiles,
                          { placeholder: true, file: placeholder },
                      ])
                    : setImages(newFiles)
            }
        },
    ...
    return (
        <FileDrop
            ...
            onDrop={handleDrop}
        >
            <Container empty={!images.length} isDragActive={isDragActive}>
                <input
                    onChange={handleDrop}
                    ref={fileInputRef}
                    type="file"
